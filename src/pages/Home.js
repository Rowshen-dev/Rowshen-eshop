...
export default function Home({ addToCart, cart, favorites, toggleFavorite, dark = true }) {
  ...
  const s = {
    hero: {
      padding: isMobile ? '40px 20px 0' : '60px 40px',
      background: 'radial-gradient(ellipse 60% 50% at 70% 50%, rgba(255,77,0,0.07) 0%, transparent 70%)',
    ...
    h1: { fontSize: isMobile ? '36px' : 'clamp(36px, 5vw, 60px)', fontWeight: 800, lineHeight: 1.05, color: '#f0f0f0', margin: '0 0 20px', letterSpacing: -2 },
    accent: { color: '#ff4d00' },
    sub: { fontSize: 15, color: '#888', margin: '0 0 32px', lineHeight: 1.7 },
    btns: { display: 'flex', gap: 12, flexWrap: 'wrap' },
    btnPrimary: { display: 'inline-flex', alignItems: 'center', gap: 8, padding: '14px 24px', background: '#ff4d00', color: '#fff', textDecoration: 'none', borderRadius: 8, fontWeight: 700, fontSize: 15 },
    btnOutline: { display: 'inline-flex', alignItems: 'center', gap: 8, padding: '14px 24px', border: '1px solid #2a2a2a', color: '#f0f0f0', textDecoration: 'none', borderRadius: 8, fontSize: 15 },
    statsRow: {
      display: 'grid',
    ...
    statItem: (i) => ({
      ...
      borderRight: isMobile
        ? (i % 2 === 0 ? '1px solid #2a2a2a' : 'none')
        : (i < 3 ? '1px solid #2a2a2a' : 'none'),
      borderBottom: isMobile && i < 2 ? '1px solid #2a2a2a' : 'none',
    }),
    statIcon: { width: 40, height: 40, background: 'rgba(255,77,0,0.08)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
    statNum: { display: 'block', fontSize: isMobile ? 18 : 22, fontWeight: 800, fontFamily: "'Space Mono', monospace", color: '#f0f0f0' },
    statLabel: { fontSize: 12, color: '#888' },
    section: { padding: isMobile ? '40px 20px' : '60px 40px' },
    sectionHead: { display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 24 },
    sectionTitle: { fontSize: isMobile ? 20 : 26, fontWeight: 800, color: '#f0f0f0', margin: 0, letterSpacing: -0.5 },
    seeAll: { fontSize: 14, color: '#ff4d00', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 4 },
    catGrid: { display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4, 1fr)', gap: 12 },
    catCard: { background: '#161616', border: '1px solid #2a2a2a', borderRadius: 12, padding: isMobile ? '20px 16px' : '28px 24px', textDecoration: 'none', transition: 'all 0.2s', display: 'block' },
    catName: { fontSize: isMobile ? 14 : 16, color: '#f0f0f0', margin: '0 0 4px', fontWeight: 700 },
    catCount: { fontSize: 12, color: '#888', margin: 0 },
    grid: { display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(auto-fill, minmax(230px, 1fr))', gap: isMobile ? 12 : 20 },
  };
  ...
  return (
    <>
      <div style={s.hero}>
        ...
        {/* 3D телефон — на мобильном под текстом */}
        <div style={s.heroRight}>
          <Suspense fallback={
            <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#555', fontSize: 14 }}>
              Загрузка...
            </div>
          }>
            <PhoneScene />
